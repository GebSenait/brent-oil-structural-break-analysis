...
def test_contributing_mentions_setup(repo_root):
    """CONTRIBUTING must include setup/run instructions (venv or Setup)."""
    contrib = (repo_root / "docs" / "CONTRIBUTING.md").read_text(encoding="utf-8")
    assert "venv" in contrib.lower() or "setup" in contrib.lower()
