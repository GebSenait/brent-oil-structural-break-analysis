...
| **Task-3** | [Description](#task-3-description) · [Implementation](#task-3-implementation) · [Test results](#task-3-test-results) · [Dashboard visualization](#task-3-dashboard-visualization) · [Insights](#task-3-insights) |
...
**Execution:** From project root: `python -m backend.app` (port 5000). Then in a **second terminal** (requires [Node.js](https://nodejs.org/) installed): `cd frontend && npm install && npm run dev`; open http://localhost:3000. Ensure `data/processed/change_point_posterior.json` exists (run Task-2 notebook) for full dashboard.
...
The React dashboard (http://localhost:3000) provides Birhan Energies–focused views of the Brent change point analysis:

- **Returns chart:** Daily Brent returns time series (Recharts line chart) with an optional vertical reference line at the posterior break date so the regime shift is visible in context.
- **Change point summary cards:** Break date (median and 95% credible interval), pre- and post-regime mean returns (μ₁, μ₂) and volatilities (σ₁, σ₂), and impact metrics (Δμ, volatility ratio) from Task-2 export.
- **Events table:** Curated geopolitical, economic, and OPEC policy events aligned to the price timeline, with a category filter (geopolitical / economic / opec_policy) for narrative context alongside the detected break.
- **Layout:** Single-page dashboard, dark theme; backend serves `data/processed/` and `change_point_posterior.json`; no notebooks or Python required for viewers.

---

## Task-3: Insights

- **Decision support:** The dashboard lets stakeholders explore Brent returns, price levels, event context by category, and the posterior change point summary for timing and strategy discussions.
- **No code required:** Regime shifts and event alignment are viewable without running notebooks or Python.
...
1. **Setup:** Clone the repo; create and activate a Python 3.10+ venv; `pip install -r requirements.txt`. See [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md) (Setup & running).
